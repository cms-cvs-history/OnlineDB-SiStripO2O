...
pedestalsTag=test_SiStripPedestals_TKCC_18X
noiseTag=test_SiStripNoise_TKCC_18X
cabTag=test_SiStripFedCabling_TKCC_18X
authPath=/afs/cern.ch/user/x/xiezhen/auth/orcon18Xowner
connectString="oracle://orcon/CMS_COND_ON_18X_STRIP"
...
sqlplus -S -M "HTML ON " cms_sitracker_tkcc/tkcc2008@omds < OMDSQuery.sql | awk 'BEGIN{newline=0; stringa=""} $0~/<*th.*>/{newline=0} $0~/<tr>/{newline=1;if(stringa!="") print stringa; stringa=""} $0~/<\/tr>/{newline=0;} $0!~/<?td.*>/{if(newline && $0!="<tr>"){stringa=sprintf("%s %s",stringa,$0)}}' | sort -n| while read line;
  do
  vec=($line)
  echo ${vec[@]}
  runNb=${vec[0]}
