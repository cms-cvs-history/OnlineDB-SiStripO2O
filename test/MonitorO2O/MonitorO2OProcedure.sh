...
function getListIOV(){
    tag=${1}_p
    cmscond_list_iov -c oracle://cms_orcoff_int2r/CMS_COND_STRIP -u CMS_COND_STRIP_R -p SSWDC3MCAI8HQHTC -f "relationalcatalog_oracle://cms_orcoff_int2r/CMS_COND_GENERAL" -t $tag | grep DB | awk '{print $1}' 
...
function GetPhysicsRuns(){
    ...
    cat log/AllRuns.txt | sort -n -k 1 | awk -F"\t" 'BEGIN{fedv=0} {if (NR>1 && ( $3 == "LATENCY" || $3 == "PHYSIC" || $3 == "PHYSIC_ZERO_SUPPRESSION" ) && fedv != $13) {fedv=$13; print $1" "$2" "$3" "$13" "$15} }'| sort -n -k 1 >> log/O2ORuns.txt
...
webpathBase=/data1/MonitorO2O/new/
...
for key in `grep TIF2 log/O2ORuns.txt `|  while read line
  do
  Run=`echo $line | cut -f 1 -d " "`
  FedV=`echo $line | cut -f 4 -d " "`
